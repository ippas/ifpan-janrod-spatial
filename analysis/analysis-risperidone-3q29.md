...
Spatial gene expression counts and associated metadata generated by Space Ranger were processed and integrated using **Seurat v4.4.0** to identify transcriptionally similar groups of spots across samples.
...
- loading gene expression counts, sample metadata and spatial information generated by Space Ranger;
...
- integration of samples using `IntegrateData` to generate a common expression space and reduce sample-specific technical differences;
...
Mean gene expression was calculated separately for each sample and cluster to generate cluster-level expression summaries. Sample-cluster combinations containing fewer than **20 spots** were excluded to avoid calculating mean expression from poorly represented clusters.
