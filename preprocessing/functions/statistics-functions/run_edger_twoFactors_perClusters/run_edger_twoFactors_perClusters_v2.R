...
#
# Fixed contrast naming and direction:
#
# Pairwise contrasts:
#   salDel_vs_salWt      = salDel / salWt
#   risDel_vs_risWt      = risDel / risWt
#   risWt_vs_salWt       = risWt / salWt
#   risDel_vs_salDel     = risDel / salDel
#   risDel_vs_salWt      = risDel / salWt
#   risWt_vs_salDel      = risWt / salDel
#
# Main effects:
#   genotype_main_del_vs_wt = average Del / WT
#   treatment_main_ris_vs_sal = average risperidone / saline
#
# Interaction:
#   interaction_genotype_x_treatment =
#   (risDel - risWt) - (salDel - salWt)
# ============================================================
...
run_edger_twoFactors_perClusters <- function(
  ...
  # risWt_vs_salDel      = risWt - salDel
  #
  # Positive log2FC means higher expression in the first group
  # of the contrast name.
  # ============================================================
  ...
  contrast_vectors <- list(
    ...
    genotype_main_del_vs_wt = make_contrast(
      c(-0.5, 0.5, -0.5, 0.5)
    ),
    
    treatment_main_ris_vs_sal = make_contrast(
      c(-0.5, -0.5, 0.5, 0.5)
    ),
    
    salDel_vs_salWt = make_contrast(
      c(-1, 1, 0, 0)
    ),
    
    risDel_vs_risWt = make_contrast(
      c(0, 0, -1, 1)
    ),
    
    risWt_vs_salWt = make_contrast(
      c(-1, 0, 1, 0)
    ),
    
    risDel_vs_salDel = make_contrast(
      c(0, -1, 0, 1)
    ),
    
    risDel_vs_salWt = make_contrast(
      c(-1, 0, 0, 1)
    ),
    
    risWt_vs_salDel = make_contrast(
      c(0, -1, 1, 0)
    )
  )
  ...
  contrast_definitions <- data.frame(
    ...
    contrast = c(
      ...
      paste0(sal_del, " - ", sal_wt),
      paste0(ris_del, " - ", ris_wt),
      paste0(ris_wt, " - ", sal_wt),
      paste0(ris_del, " - ", sal_del),
      paste0(ris_del, " - ", sal_wt),
      paste0(ris_wt, " - ", sal_del)
    ),
    stringsAsFactors = FALSE
  )
