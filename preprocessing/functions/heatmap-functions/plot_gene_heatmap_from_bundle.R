...
plot_gene_heatmap_from_bundle <- function(
  ...
  border_lwd  = 1,
  row_names_simple = TRUE,
  return_matrix = FALSE   # 👈 NOWY ARGUMENT
){
  stopifnot(is.list(bundle), "metadata" %in% names(bundle))
  data_type <- match.arg(data_type)
  ...
  # 👈 ZWRACAMY MACIERZ PRZED Z-SCORE
  if (return_matrix) {
    message("📤 Returning matrix before z-score normalization.")
    return(heat_mat)
  }
...
plot_gene_heatmap_from_bundle <- function(
  ...
  # ---- Imputation ----
  if (isTRUE(impute_by_group)) {
    message("🧩 Imputing missing values by group mean...")
    df_long <- df_long %>%
      dplyr::group_by(group, peak_id, cluster) %>%
      dplyr::mutate(
        value = ifelse(
          is.na(value),
          ifelse(all(is.na(value)), 0, mean(value, na.rm = TRUE)),
          value
        )
      ) %>%
      dplyr::ungroup()
  ...
  # ---- Cluster order (numeric sort) ----
  if (is.null(cluster_order)) {
    cluster_levels <- unique(peaks_df[[cluster_col]])
    nums <- suppressWarnings(as.numeric(sub(".*?(\\d+)$", "\\1", cluster_levels)))
    cluster_order <- cluster_levels[order(nums, na.last = TRUE)]
  }
  ...
  # >>> KLUCZ: row_split jako FAKTOR z poziomami = cluster_order <<<
  row_split <- if (isTRUE(divide_by_cluster)) {
    factor(sub("\\|.*", "", rownames(heat_mat)), levels = cluster_order)
  } else {
    NULL
  }
  
  column_split <- if (isTRUE(divide_columns_by_groups) && length(group_cols) > 0) {
    interaction(annot_col[, group_cols, drop = FALSE], sep = "_", drop = TRUE)
  } else NULL
  ...
  ht <- ComplexHeatmap::Heatmap(
    ...
    column_split = column_split,
    column_gap = column_gap,
    row_split = row_split,  # <- teraz wymusi kolejność klastrów wg cluster_order
    row_gap = if (isTRUE(divide_by_cluster)) row_gap else grid::unit(0, "mm"),
    row_dend_width = row_dend_width,
    row_title_rot = row_title_rot,
    row_title_gp = grid::gpar(fontsize = 10, fontface = "bold"),
...
plot_gene_heatmap_from_bundle <- function(
  ...
  return_matrix = FALSE,
  impute_by_group = TRUE,
  add_marker_for_imputed_data = TRUE  # 👈 kropki dla imputacji
){
  stopifnot(is.list(bundle), "metadata" %in% names(bundle))
  data_type <- match.arg(data_type)
  ...
  if (isTRUE(impute_by_group)) {
    ...
    df_long <- df_long %>%
      dplyr::group_by(group, peak_id, cluster) %>%
      dplyr::mutate(
        imputed = is.na(value),  # znacznik imputacji (przed nadpisaniem)
        value = ifelse(
          is.na(value),
          ifelse(all(is.na(value)), 0, mean(value, na.rm = TRUE)),
          value
  ...
  # ---- Build heatmap and imputation matrices ----
  heat_mat <- df_long %>%
    dplyr::select(peak_label, sample_ID, value) %>%
    dplyr::group_by(peak_label, sample_ID) %>%
    dplyr::summarise(value = mean(value, na.rm = TRUE), .groups = "drop") %>%
  ...
  imputed_mat <- df_long %>%
    dplyr::select(peak_label, sample_ID, imputed) %>%
    dplyr::group_by(peak_label, sample_ID) %>%
    dplyr::summarise(imputed = any(imputed), .groups = "drop") %>%
    tidyr::pivot_wider(names_from = sample_ID, values_from = imputed, values_fill = FALSE) %>%
    tibble::column_to_rownames("peak_label") %>%
    as.matrix()
  ...
  common_samples <- intersect(sample_order, colnames(heat_mat))
  heat_mat     <- heat_mat[, common_samples, drop = FALSE]
  imputed_mat  <- imputed_mat[, common_samples, drop = FALSE]
  annot_col    <- annot_col[common_samples, , drop = FALSE]
  ...
  # --- cell_fun: marker dla imputowanych komórek (działa po re-orderingu) ---
  cell_fun_arg <- NULL
  if (isTRUE(add_marker_for_imputed_data)) {
    cell_fun_arg <- function(j, i, x, y, w, h, col) {
      rn <- rownames(heat_mat)[i]
      cn <- colnames(heat_mat)[j]
      if (!is.na(imputed_mat[rn, cn]) && isTRUE(imputed_mat[rn, cn])) {
        grid::grid.points(x, y, pch = 16, size = grid::unit(1.5, "mm"),
...
plot_gene_heatmap_from_bundle <- function(
  ...
  exclude_samples = NULL   # 👈 NOWY ARGUMENT
){
  stopifnot(is.list(bundle), "metadata" %in% names(bundle))
  data_type <- match.arg(data_type)
  ...
  if (!is.null(exclude_samples)) {
    message("🚫 Excluding ", length(exclude_samples), " samples: ",
  ...
  # ---- Apply exclusion to annotation ----
  if (!is.null(exclude_samples)) {
    annot_col <- annot_col %>%
      dplyr::filter(!rownames(.) %in% exclude_samples)
  }
  ...
  # --- cell_fun: marker dla imputowanych komórek ---
  cell_fun_arg <- NULL
  if (isTRUE(add_marker_for_imputed_data)) {
    cell_fun_arg <- function(j, i, x, y, w, h, col) {
      rn <- rownames(heat_mat)[i]
