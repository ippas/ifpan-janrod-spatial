...
server <- function(input, output, session) {
  ...
  output$sample_display_block_expr <- renderUI({
    ...
    control_name <- dataset_config()$control_samples
    case_name <- dataset_config()$case_samples
    
    control <- if (exists(control_name, envir = .GlobalEnv)) {
      get(control_name, envir = .GlobalEnv)
    } else {
      character(0)
    }
    
    case <- if (exists(case_name, envir = .GlobalEnv)) {
      get(case_name, envir = .GlobalEnv)
    } else {
      character(0)
    }
  ...
  observeEvent(input$dataset, {
    # Spatial data name (string)
    spatial_data_name <- dataset_config()$spatial_data
    
    # Ustaw spatial_data do reactives
    updateTextInput(session, "spatial_data", value = spatial_data_name)
    ...
    # Jeśli obiekty z próbkami istnieją — załaduj je
    control_var <- dataset_config()$control_samples
    case_var <- dataset_config()$case_samples
    
    if (exists(control_var, envir = .GlobalEnv)) {
      control <- get(control_var, envir = .GlobalEnv)
      updateCheckboxGroupInput(session, "control_selected_expr",
                               choices = control,
                               selected = control)
    }
    
    if (exists(case_var, envir = .GlobalEnv)) {
      case <- get(case_var, envir = .GlobalEnv)
      updateCheckboxGroupInput(session, "case_selected_expr",
                               choices = case,
                               selected = case)
    }
    
    # Update cluster selections
    lapply(0:2, function(col_index) {
      updateCheckboxGroupInput(
        session,
        inputId = paste0("cluster_selector_expr_col", col_index + 1),
    ...
    vals$resolution_expr <- 0.4
    vals$select_clusters_expr <- as.character(0:29)
  })
  ...
  output$expr_spatial_plot_cluster <- renderPlot({
    ...
    # 1. Sprawdź dataset i dane wymagane
    req(input$dataset)
    req(input$data_type_visualization_expr)
    req(input$gene_selection_expr)
    req(input$peak_selection_expr)
    
    selected_samples <- c(input$control_selected_expr, input$case_selected_expr)
    if (length(selected_samples) == 0) {
      plot.new(); text(0.5, 0.5, "No samples selected.", cex = 1.5, col = "red"); return()
    }
    
    if (is.null(vals$select_clusters_expr) || length(vals$select_clusters_expr) == 0) {
      plot.new(); text(0.5, 0.5, "No clusters selected.", cex = 1.5, col = "red"); return()
    }
    
    if (is.null(vals$resolution_expr)) {
      plot.new(); text(0.5, 0.5, "Resolution not set.", cex = 1.5, col = "red"); return()
    }
