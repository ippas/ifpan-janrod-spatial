    ...
    tabPanel("🧬 Gene Expression (Barplot)",
             sidebarLayout(
               sidebarPanel(
                 h4("Visualization Options"),
                 fluidRow(
                   column(6, numericInput("width_plot_bar", "Width (px):", value = 1300, min = 100, step = 50)),
                   column(6, numericInput("height_plot_bar", "Height (px):", value = 950, min = 100, step = 50))
                 ),
                 fluidRow(
                   column(6, selectInput("num_columns_bar", "Number of Columns", choices = 1:7, selected = 4)),
                   column(6, numericInput("dpi_svg_bar", "SVG DPI:", value = 100, min = 50, max = 600, step = 10))
                 ),
                 checkboxInput("same_y_scale_bar", "Same Y Scale", value = FALSE),
                 tags$hr(),
                 h4("Selecting Data"),
                 fluidRow(
                   column(4,
                          selectInput("stat_summary_metric_bar", "Select metric:",
                                      choices = c("mean", "median", "sum"),
                                      selected = "mean")
                   ),
                   column(4,
                          selectInput("stat_data_type_bar", "Select data type:",
                                      choices = c("raw_data", "seurat", "range_normalize", "quantile_metric", "quantile_normalize"),
                                      selected = "raw_data")
                   ),
                   column(4,
                          selectInput("stat_resolution_bar", "Select resolution:",
                                      choices = c("0.05", "0.1", "0.15", "0.2", "0.4", "0.8", "1"),
                                      selected = "0.4")
                   )
                 ),
                 tags$hr(),
                 h4("Statistical Test"),
                 selectInput("stat_test_type_bar", "Test:",
                             choices = c("T-Student", "Wilcoxon", "Kolmogorov-Smirnov"),
                             selected = "T-Student"),
                 tags$hr(),
                 h4("Gene & Peak Selection"),
                 uiOutput("data_type_visualization_bar"),
                 uiOutput("gene_selector_bar"),
                 uiOutput("peak_selector_bar"),
                 tags$hr(),
                 h4("Cluster Selection"),
                 div(style = "display: flex; gap: 6px; margin-bottom: 8px;",
                     actionButton("select_all_clusters_bar", "Select All"),
                     actionButton("deselect_all_clusters_bar", "Deselect All")
                 ),
                 fluidRow(
                   lapply(0:2, function(col_index) {
                     column(4,
                 ...
                 br(),
                 actionButton("update_resolution_bar", "Update Resolution and Cluster"),
                 tags$hr(),
                 downloadButton("downloadBarplotPNG", "Download PNG"),
                 downloadButton("downloadBarplotSVG", "Download SVG")
               ),
               mainPanel(
                 h4("Barplot of Expression"),
                 plotOutput("barplot_expr", height = "600px")
               )
             )
    )
