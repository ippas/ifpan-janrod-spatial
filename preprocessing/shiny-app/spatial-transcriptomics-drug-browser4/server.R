...
server <- function(input, output, session) {
  ...
  selected_saline_samples <- eventReactive(input$update_samples, {
    saline_ids <- c( "S6269Nr1",  "S6269Nr3",  "S7788Nr1",  "S7788Nr2",  "S7788Nr3",  "S7788Nr11",
                     ...
                     "S13839Nr1", "S13839Nr5", "S13839Nr12", "S13839Nr20", "S13839Nr22",
                     "S13839Nr3",  "S13839Nr6",  "S13839Nr11", "S13839Nr14", "S13839Nr18",
                     "S13839Nr26"
                     )
    selected <- lapply(saline_ids, function(x) {
  ...
  observeEvent(input$drug, {
    ...
    if (input$drug == "Risperidone") {
    ...
    }  else if (input$drug == "risWtSalWt") {

      
      updateSelectInput(session, "spatial_data",
                        choices = c("ris3q29_st_data"),
                        selected = "ris3q29_st_data")
      
      updateSelectInput(session, "summary_data",
                        choices = c("risWtSalWt_summary_statistics"),
                        selected = "risWtSalWt_summary_statistics")
      
      updateSelectInput(session, "experiment_samples",
                        choices = c("samples_risWt"),
                        selected = "samples_risWt")
      
    } else if (input$drug == "risDelSalDel") {
      ...
      updateSelectInput(session, "summary_data",
                        choices = c("risDelSalDel_summary_statistics"),
                        selected = "risWtDelDel_summary_statistics")
      
      updateSelectInput(session, "experiment_samples",
                        choices = c("samples_risDel"),
                        selected = "samples_risDel")
  ...
  drug_samples <- list(
    ...
    "risWtSalWt" = c("S13839Nr4", "S13839Nr10", "S13839Nr13", "S13839Nr16", "S13839Nr24", "S13839Nr25"),
    "risDelSalDel" = c("S13839Nr2", "S13839Nr7", "S13839Nr8", "S13839Nr9", "S13839Nr15", "S13839Nr17", "S13839Nr19", "S13839Nr21", "S13839Nr23")
  )
