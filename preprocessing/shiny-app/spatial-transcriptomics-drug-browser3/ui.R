...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("drug", "Select Drug", choices = c("Risperidone", "PZ-1190", "Clozapine", "Ldopa")),
      ...
      fluidRow(
        ...
        column(6,
               h5(tags$b("Saline")),
               lapply(c("S5023Nr1", "S5023Nr4", "S5295Nr1", "S5295Nr6", "S5295Nr7"),
                      function(x) checkboxInput(inputId = paste("saline_", gsub("Nr|S", "", x), sep = ""),
                                                label = x, value = F))),
        column(6,
               h5(tags$b("Ldopa")),
               lapply(c("S5023Nr2", "S5023Nr3", "S5295Nr3", "S5295Nr8"),
                      function(x) checkboxInput(inputId = paste("experimental_", gsub("Nr|S", "", x), sep = ""),
                                                label = x, value = F)))
      ),
