generate_cluster_heatmap <- function(data, genes, cluster_name, metadata,
  ...
  # 3) Usunięcie wierszy całkowicie pustych (opcjonalnie)
  df_cluster <- df_cluster %>%
    dplyr::group_by(peak) %>%
    dplyr::filter(
      # usuń wiersze, gdzie wszystkie wartości NA
      !(remove_empty_rows && base::all(base::is.na(scaled_expression)))
    ) %>%
    dplyr::ungroup()
  
  # 4) Przygotowanie meta2 z dynamicznym grupowaniem
  ...
  # 5) Budowa macierzy ekspresji w porządku meta2
  mat <- df_cluster %>%
  ...
  # 6) Przygotowanie Heatmap arguments z możliwością tytułu i zakresu skalowania
  hm_args <- list(
    mat2,
    name                 = paste0(toupper(substring(summary_metric,1,1)), substring(summary_metric,2), " Z-score"),
    col                  = circlize::colorRamp2(c(scale_range[1], 0, scale_range[2]), c("blue","white","red")),
    na_col               = "grey80",
    show_row_names       = TRUE,
    row_names_side       = "left",
  ...
  # 7) Dodanie adnotacji z kolorami grup
