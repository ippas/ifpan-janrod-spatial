...
#
# Purpose:
#   Calculate two cross-cell contrasts in a 2x2 design:
#
#   1) risDel vs salWt
...
#   The legacy input file name contains "pInteraction0.05".
#   This is a historical naming artifact only.
#   The file was verified on 2026-06-29 to contain full edgeR results and is
#   NOT filtered by P_interaction < 0.05, FDR, or |logFC|.
#   It contains features retained after the standard edgeR low-expression
#   filterByExpr() step.
#
# Final output:
#   Full edgeR results with means and two additional cross-cell contrasts.
#   No final filtering by P-value, FDR, or logFC is performed.
# ##############################################################################
...
results_dir <- "/home/mateusz/projects/ifpan-janrod-spatial/results/risperidone-3q29/edger_statistics"

# Legacy input filename: "pInteraction0.05" is only a naming artifact.
input_file <- file.path(
  results_dir,
  "edgerMarginalProportional_combineMultiComparision_pInteraction0.05_withMeans_09.06.2026.xlsx"
)

# New correctly named full-results output.
output_file <- file.path(
  results_dir,
  "edgerMarginalProportional_combineMultiComparision_withMeans_fullResults_29.06.2026.xlsx"
)
...
run_edger_two_cross_cell_contrasts <- function(
  ...
  # Standard low-expression filtering before model fitting.
  # This is not a final filter by statistical significance or effect size.
...
# ==============================================================================
# 7. Quick checks: cross-cell contrasts
# ==============================================================================

dim(df_cross_cell_contrasts)
...
# ==============================================================================
# 8. Read full legacy edgeR results table
# ==============================================================================

df_edger_marginal_proportional <- readxl::read_excel(
  input_file,
  sheet = 1
)

df_edger_marginal_proportional %>%
  summarise(
    n_rows = n(),
    min_P_interaction = min(P_interaction, na.rm = TRUE),
    median_P_interaction = median(P_interaction, na.rm = TRUE),
    max_P_interaction = max(P_interaction, na.rm = TRUE)
  )


# ==============================================================================
# 9. Prepare new cross-cell contrasts for joining
# ==============================================================================
...
# ==============================================================================
# 10. Join new contrasts to the full results table
# ==============================================================================
...
# ==============================================================================
# 11. Move newly added contrast columns before mean_* columns
# ==============================================================================
...
# ==============================================================================
# 12. Quick checks: final full-results table
# ==============================================================================
...
# ==============================================================================
# 13. Save final full-results table
# ==============================================================================
...
message("Saved full edgeR results to:")
message(output_file)
